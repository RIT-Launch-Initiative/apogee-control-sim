...
Q4=optimizableVariable("Q4",[0 30]);
Q5=optimizableVariable("Q5",[0 30]);
vars=[Q1 Q2 Q3 Q4 Q5];
...
figure();
title("Metrics of Tuning Improvement");
alt_rmse=[];
...
alt_v_std=[];
for i=1:num_solver_steps
    alt_v_std(end+1,1)=1-std(alt_v(i,:));
end
...
% y=y./max(y);
% figure;
% scatter3(data.Q1, data.Q2, data.Q3, 50, y, 'filled');
% colorbar;
...
function cost=wrapper_func(vars,simin)
    ...
    % out=out+trapz(lags,r);

    cost=rmse(logs.altitude_meas,logs.altitude_est)*4;
    cost=cost+rmse(logs.velocity_meas,logs.velocity_est)*4;
    cost=cost+rmse(logs.accel_meas,logs.accel_est)*4;
    % [r,lags]=xcorr(logs.v_innovation(:,1),"normalized");
    % cost=cost+trapz(lags,r)*2;
    % [r,lags]=xcorr(logs.v_innovation(:,2),"normalized");
    % cost=cost+trapz(lags,r)*2;
    cost=cost+(1-std(logs.innovation_norm(:,1)))*10
    cost=cost+(1-std(logs.innovation_norm(:,2)))*10;
