...
brake_off.control_brake = false;
brake_off.const_brake = 0;

brake_on.control_brake = false;
brake_on.const_brake = 0;

simin = struct2input(sim_file, vehicle_data);
simin = struct2input(simin, inits);

simin_off = struct2input(simin, struct(control_brake = false, const_brake = 0));
simin_on = struct2input(simin, struct(control_brake = false, const_brake = 1)); % max braking effort
...
simin_opt = struct2input(simin, struct(control_brake = false, const_brake = ext));
